<template>
  <div>
    ...
    >
      ...
      <audio
        ref="audioTestRef"
        src="/assets/sounds/speaker-test.mp3"
        preload="auto"
      ></audio>
      <div
        v-if="!isCameraOn"
...
const testSpeakers = async () => {
  ...
  try {
    ...
    try {
      ...
      if (playErr instanceof Error) {
        if (playErr.name === "NotAllowedError") {
          alert(
            "Could not play audio test automatically. Browser restrictions require user interaction first."
          );
        } else {
        ...
        // Optional: handle non-Error exceptions
        console.error("Unknown error during audio test:", playErr);
    ...
    alert(
      "Could not test speakers. This feature may not be supported in your browser."
    );
  }
};
